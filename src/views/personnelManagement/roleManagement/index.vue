  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <el-input
            v-model.trim="dialog.dataRow.rolename"
...
import { addRole, updateRole } from '@/api/page/role.js'
...
export default {
  ...
  data () {
    return {
      ...
      tableOptions: {
        column: [
          { prop: 'rolename', label: '角色名', width: '220', sortable: true },
          {
            label: '备注',
            prop: 'remark'
          },
  ...
  methods: {
    getConditions (conditions) {
      if (this.queryForm.rolename && this.queryForm.rolename.length > 0) {
        conditions.push({ field: 'rolename', type: '包含', value: this.queryForm.rolename })
      }
      return conditions
    },
    getDetailsConditions (conditions) {
      conditions.push({ field: 'roleids', value: this.detailsDia.dataRow.id, type: '包含'})
      return conditions
    },
    searchData () {
          ...
          if (this.dialog.type === 'edit') {
            updateRole({
              ...
              dataList: JSON.stringify({
                ...
                rolename: this.dialog.dataRow.rolename,
                modules: modules,
              }),
            })
              .then((res) => {
